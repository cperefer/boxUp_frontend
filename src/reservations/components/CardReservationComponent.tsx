...
export const CardReservationComponent = ({ id, type, date, title }: Props) => {
  return (
    <div className="flex flex-row border-2 border-cyan-100 shadow-md min-h-25 rounded-2xl">
      <div
        className={`hidden md:flex w-1/3 h-full rounded-l-2xl ${classColors[type]} items-center justify-center`}
      >
        <IconContext.Provider
          value={{ color: "white", className: "card-icon" }}
        >
          <div>{classIcons[type]}</div>
        </IconContext.Provider>
